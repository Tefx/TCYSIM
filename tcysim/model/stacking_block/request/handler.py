from tcysim.framework import ReqHandlerBase, EventReason
from tcysim.framework.exception import RORBoxBeingOperatedError, ReqOpRejectionError
...
class ReqHandlerForStackingBlock(ReqHandlerBase):
    ...
    def on_request_store(self, time, request):
        ...
        yield self.equipment.op_builder.StoreOp(request)
    ...
    def on_request_adjust(self, time, request):
        ...
        yield self.equipment.op_builder.AdjustOp(request)
    ...
    def on_reject(self, time, e):
        if isinstance(e, RORAcquireFail):
            ...
            super(ReqHandlerForStackingBlock, self).on_reject(time, e)
