...
DLLEXPORT int box_relocate(Box *box, CellIdx *new_loc);

DLLEXPORT int box_relocate_retrieve(Box *box, CellIdx *new_loc);

DLLEXPORT bool box_position_is_valid(Box *box, Block *blk, CellIdx *loc);

DLLEXPORT void box_store_position(Box *box, CellIdx *idx);

DLLEXPORT void box_relocate_position(Box *box, CellIdx *new_loc);
